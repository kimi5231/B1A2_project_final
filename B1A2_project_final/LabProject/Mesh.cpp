...
{
	// 정점 버퍼 뷰와 인스턴싱 버퍼 뷰를 입력-조립 단계에 설정
...
CTriangleMesh::CTriangleMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList) : CMesh(pd3dDevice, pd3dCommandList)
{
	...
	// RGBA(Red, Green, Blue, Alpha) 4개의 파라미터를 사용하여 색상을 표현함
	// 각 파라미터는 0.0~1.0 사이의 실수값을 가짐
...
: CMesh(pd3dDevice, pd3dCommandList)
{
	...
	// 인덱스 버퍼는 직육면체의 6개의 면(사각형)에 대한 기하 정보를 갖음
	// 삼각형 리스트로 직육면체를 표현할 것이므로 각 면은 2개의 삼각형을 가지고 각 삼각형은 3개의 정점이 필요함.
