...
void CGameFramework::CreateDirect3DDevice()
{
	...
	// 레벨 12.0을 지원하는 하드웨어 디바이스를 생성할 수 없으면 WARP 디바이스 생성
	if (!m_pd3dDevice)
	{
	...
	// 다중 샘플링 품질 수준이 1보다 크면 다중 샘플링 활성화
	...
	// 펜스 동기화를 위한 이벤트 객체 생성(초기값 FALSE)
	// 이벤트가 실행되면(Signal) 이벤트 값을 자동적으로 FALSE가 되도록 생성
...
void CGameFramework::CreateCommandQueueAndList()
{
	...
	// 명령 리스트는 생성되면 열린 상태이므로 닫힌 상태로 만듦
	hResult = m_pd3dCommandList->Close();
}

void CGameFramework::CreateRtvAndDsvDescriptorHeaps()
{
	// 렌더타겟 디스크립터 힙 생성(디스크립터의 개수는 스왑체인 버퍼의 개수)
	...
	// 렌더 타켓 디스크리버 힙의 원소 크기 저장
	m_nRtvDescriptorIncrementSize = m_pd3dDevice->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);

	// 깊이-스텐실 디스크립터 힙 생성(디스크립터 개수는 1)
...
void CGameFramework::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYUP:
		...
		case VK_RETURN:
			break;
		//‘F1’ 키를 누르면 1인칭 카메라,
		// 'F2’ 키를 누르면 스페이스-쉽 카메라
		// 'F3’ 키를 누르면 3인칭 카메라로 변경
		case VK_F1:
		case VK_F2:
		case VK_F3:
...
void CGameFramework::ProcessInput()
{
	...
	if (!bProcessedByScene)
	{
		...
		{
			// 마우스 커서를 화면에 보이지 않도록 설정
			::SetCursor(NULL);
		
			::GetCursorPos(&ptCursorPos);
			
			// 마우스 버튼이 눌린 상태에서 움직인 양
		...
		//마우스 또는 키 입력이 있으면 플레이어를 이동하거나(dwDirection) 회전함(cxDelta 또는 cyDelta)
		if ((dwDirection != 0) || (cxDelta != 0.0f) || (cyDelta != 0.0f))
		{
			// cxDelta는 y-축의 회전을 나타내고 cyDelta는 x-축의 회전을 나타냄
...
void CGameFramework::FrameAdvance()
{
	...
	// 현재 렌더 카겟에 대한 프리젠트가 끝나기를 기다림
	// 프리젠트가 끝나면 렌더 타겟 버퍼의 상태는 프리젠트 상태에서 렌더 타겟 상태로 바뀔 것
	...
	// 현재의 렌더 타켓에 해당하는 디스크립터의 CPU 주소(핸들) 계산
	D3D12_CPU_DESCRIPTOR_HANDLE d3dRtvCPUDescriptorHandle = m_pd3dRtvDescriptorHeap->GetCPUDescriptorHandleForHeapStart();
