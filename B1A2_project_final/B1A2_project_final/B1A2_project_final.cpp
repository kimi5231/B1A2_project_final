                     ...
                     _In_ int       nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_B1A2PROJECTFINAL, szWindowClass, MAX_LOADSTRING);
    
    // 1. 윈도우 클래스 등록
    MyRegisterClass(hInstance);

    // 2. 응용 프로그램 초기화(주 윈도우 생성)
    if (!InitInstance (hInstance, nCmdShow))
    {
        return FALSE;
    }

    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_B1A2PROJECTFINAL));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.lpszMenuName = nullptr;   // 메뉴 없앰
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   // 주 윈도우가 생성되면, CGameFramework 객체를 초기화
