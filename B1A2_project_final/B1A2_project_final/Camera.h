...
public:
	...
	//3인칭 카메라에서 카메라가 바라보는 지점을 설정. 일반적으로 플레이어를 바라보도록 설정함
	virtual void SetLookAt(XMFLOAT3& xmf3LookAt) {}

	// 절두체(월드 좌표계)를 생성
	void GenerateFrustum();
	// 바운딩 박스(OOBB, 월드 좌표계)가 절두체에 포함되는가를 검사
...
protected:
	...
	// 카메라의 로컬 x축(Right), y축(Up), z축(Look) 벡터
	XMFLOAT3 m_xmf3Right;
	XMFLOAT3 m_xmf3Up;
	XMFLOAT3 m_xmf3Look;

	// 카메라가 x축, y축, z축으로 얼만큼 회전했는가를 나타내는 각도
	float m_fPitch;
	float m_fRoll;
	float m_fYaw;
	...
	//플레이어가 바라볼 위치 벡터. 주로 3인칭 카메라에서 사용됨
	XMFLOAT3 m_xmf3LookAtWorld;

	//플레이어와 카메라의 오프셋을 나타내는 벡터. 주로 3인칭 카메라에서 사용됨
